...
def encode_pkts(msg, pkts, morse_code, offset=0):
    ...
    print("start encode in TCP packets: {}".format(len(pkts[TCP])))
    ...
    for p in pkts[TCP][offset:]:
        if not char:
            try:
                ...
                p.reserved = morse_code['space']
                continue

        p.reserved = int(char.popleft())

    return pkts

def decode_pkts(pkts, morse_code):

    recv = [int(p[TCP].reserved) for p in pkts[TCP]]
    morse = [v for v in morse_code.values()]

    print("Start decode in TCP packets: {}".format(len(pkts[TCP])))

    mark = ''.join([str(i) for i in morse_code['1']])
    # print(recv)
    msg = []
    morse_msg = re.search(mark+'(.+)'+mark, ''.join([str(i) for i in recv])).group(1)
    # check for intervalled message
...
def main():
    ...
    parser.add_argument('--dot', dest='dot', action='store', type=int, default=1)
    parser.add_argument('--dash', dest='dash', action='store', type=int, default=2)
    parser.add_argument('--space', dest='space', action='store', type=int, default=8)
    ...
    group_dec.add_argument('-o', '--file-output', dest='out', action='store')
